...
def cif_to_xyz(cif, out):
    ...
    sga = pmg.symmetry.analyzer.SpacegroupAnalyzer(structure)
    std_structure = sga.get_conventional_standard_structure()
    text = str(std_structure)

    sitios = int(text.split('\n')[4].split('(')[1].split(')')[0])

    abc = [float(item) for item in list(
        filter(None, text.split('\n')[2].split(':')[1].split(' ')))]
    angles = [float(item) for item in list(
        filter(None, text.split('\n')[3].split(':')[1].split(' ')))]
    lista = text.split('\n')[-sitios:]
    ...
    motif[4] = 1*(motif[1].values == 0)+1 * \
        (motif[2].values == 0)+1*(motif[3].values == 0)
    ...
    volume = abc[0]*abc[1]*abc[2]*np.sqrt(1-(np.cos(np.deg2rad(angles[0])))**2-(np.cos(np.deg2rad(angles[1])))**2-(np.cos(
        np.deg2rad(angles[2])))**2+2*np.cos(np.deg2rad(angles[0]))*np.cos(np.deg2rad(angles[1]))*np.cos(np.deg2rad(angles[2])))

    matrix = np.array([[abc[0], abc[1]*np.cos(np.deg2rad(angles[2])), abc[2]*np.cos(np.deg2rad(angles[1]))],
                       [0, abc[1]*np.sin(np.deg2rad(angles[2])), abc[2]*(np.cos(np.deg2rad(angles[0]))-np.cos(
                           np.deg2rad(angles[1]))*np.cos(np.deg2rad(angles[2])))/np.sin(np.deg2rad(angles[2]))],
                       [0, 0, volume/(abc[0]*abc[1]*np.sin(np.deg2rad(angles[2])))]])
    ...
    positions['element'] = positions['element'].map(
        lambda x: x.lstrip('+-').rstrip('0123456789:.'))
    positions['element'] = positions['element'].map(
        lambda x: x.lstrip('0123456789.:').rstrip('+-:.'))
